...
label = Label(root, text="Push-Up Count: 0", font=("Arial", 24))
label.pack(pady=10)
...
def update_push():
    global count
    label.config(text=f"PUSH-UPS: {count}")
    label.after(1000, update_push)

# Function to update the push-up count every second
...
def process_frame():
    ...
    if result.pose_landmarks:
        ...
        if len(imlist) != 0:
            # Check the positions of shoulders and elbows
            if imlist[11][2] > imlist[13][2] and imlist[12][2] > imlist[14][2]:  # Shoulders above elbows (down position)
                position = "down"
            if imlist[11][2] < imlist[13][2] and imlist[12][2] < imlist[14][2] and position == "down":  # Shoulders below elbows (up position)
                position = "up"
                count += 1
                print(count)
...
# Function to open the video captured for detection of push-ups

def open_video():
    global cap
    cap.release()
    cap = cv2.VideoCapture("PushUps.mkv")  # Replace with your video file if needed
...
# Updating the push-up count and processing video frames
